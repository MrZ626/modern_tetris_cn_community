...
不过代码里发现了一个公式，应该是完整算法中的重要组成：`max(0, 受击权重 + (位于榜一?2:0) - 受击保护*(受击保护+5)/50)`

你可以在这个[Desmos文件](https://www.desmos.com/calculator/xjfkfkdimn)里观察一下图像，还可以调整参数看看随不同值如何变化，虽然并不知道有啥用
