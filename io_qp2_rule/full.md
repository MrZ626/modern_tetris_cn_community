...
> 为什么有个`E`：在代码中有一个windup_until计时器，吃到多段攻击时被设为了`2+0.5*段数`(秒)  
> 注意到多数情况下，从`S`开始直到`E`结束的期间不会再收到普通攻击，故猜测它的功能类似无敌计时器  
> 服务端在给玩家分配攻击时会参考本地的模拟玩家对象的该计时器若未归零就略过本次（无法验证，需要服务端代码）
