...
|   发送   | `行数`m |       `行数+0.05`经验        |                                                                                      |
|   抵消   |         |       `行数+0.05`经验        | 【(r)EX】【rVL】【r2P】时视为抵消0行                                                 |
...
玩家有一个`受击权重`状态会影响自身被攻击的概率，**初始值为3**，越大越容易被其他人攻击
...
### 垃圾行保护（默认关闭，仅在部分Mod开启时使用，详见后文）

场地内的每一行`非永久垃圾行`（包含可消除的***灰色***格子的行）会使`受击权重`减少0.5，至多减2.5，每0.25秒刷新一次
...
### 攻击切片 & 攻击犹豫（Hesitated Attack）

`切片`：任何玩家的单次攻击超过4时会切分成最大4的单元再发送，例如10会被拆成4+4+2同时发出

`犹豫`：由于有切片机制，玩家可以在同一帧内收到来自一个人的多次攻击。如果在同一帧内收到了来自同一人的攻击，会先将其合并再生成红条

上述两个机制在通常情况下是透明（无法被观测到）的，例如A发出4+3都打给了B，B就会将其合并成7再生成红条

可以推测这两个系统的目的是：服务器在处理例如100surge转化为的25x4攻击时，哪怕这25组攻击是一瞬间的，服务器也可以在中途切换玩家的攻击目标，从而实现不让100攻击全打给同一个人

同时结合诸如上文的`受击权重`可以继续推测：在分配攻击时，服务器有理由直接根据`受击保护`和`受击权重`等值来决定/切换攻击对象
...
预警动画：收到多段攻击时先播放动画和提示音，提示音数量对应攻击有几段
...
S：开始阶段，播放感叹号动画，占用1s
A：攻击阶段，每段攻击占用0.5s
E：结束阶段，占用1s，作用不明，在代码中有一个windup_until计时器，吃到多段攻击时被设为了`2+0.5*段数`(秒)，对应这个阶段的结束时的时间点
```
...
玩家的攻击对象由服务器全权随时决定，完整算法未知。

不过在代码里发现了一个debug用的公式：`Tgt.Factor = max(0, 受击权重 + (位于榜一?2:0) - 受击保护*(受击保护+5)/50)`

这个公式大概率是完整算法的一部分，决定了真实的攻击对象选择逻辑，例如在结算25组4点攻击的过程中，每送出一组会刷新一下被攻击者的最终Factor，再去决定下一组攻击给谁

但不能排除服务端有修改，或者后续还有其他步骤，所以该公式仅供参考

你可以在这个[Desmos文件](https://www.desmos.com/calculator/xjfkfkdimn)里观察一下图像，还可以调整参数看看如何随不同值变化，虽然用处不大
...
Mod总共有9个，每个都对应一个特殊效果可以独立开关，在背景设定中呈现为塔罗牌，后文会在每个Mod的名称后注明，章节最后也有一个表格用于速查
...
【rEX】和【r2P】下，攻击在最终发出前会被/2并向下取整（不影响经验等值的计算），应该是因为这两个模式“正常游玩”时输出过多，需要平衡强度

如果你能看懂javascript代码：[技术信息补充](技术信息补充.js)

qp2模式其实就是一个巨大的房间，房间号是“x-qp”，输入房间号进入和通过菜单按钮进入的效果相同

根据过往发生的一些情况，该房间有可能有“匿名用户数据超过一定值后会被系统自动ban”的机制，详细触发条件不明，不能排除是管理员手动操作
...
感谢 ThTsOd 提供格式化代码的方案等帮助和部分片段解读
